package com.ly.ly_base_network.code;
...
import com.ly.ly_base_network.code.errorhandler.ExceptionHandle;
import com.ly.ly_base_network.code.errorhandler.HttpErrorHandler;
import com.ly.ly_base_network.code.interceptor.RequestInterceptor;
import com.ly.ly_base_network.code.interceptor.ResponseInterceptor;
...
public class NetworkApi {
     ...
     * 创建serviceClass的实例
     * @param serviceClass service的类名
     * @param baseUrl 网络访问的基础地址，可为空，为空时会取network.config中配置的baseUrl字段值
     */
    ...
    private static void getBaseUrl() {
        ...
        String appConfigStr = parseFile("networkConfig.json", iNetworkRequiredInfo.getApplicationContext());

        if (!appConfigStr.isEmpty()) {
            Gson gson = new Gson();
            NetworkConfig networkConfig = gson.fromJson(appConfigStr, new TypeToken<NetworkConfig>() {}.getType());

            if (networkConfig != null && networkConfig.baseUrl != null && !networkConfig.baseUrl.isEmpty()) {

                mBaseUrl = networkConfig.baseUrl;
                return;
            }
        }
        mBaseUrl = "https://com.ly.wanandroidly";
    }
    ...
    private static String parseFile(String fileName, Context context) {
